...
" You should test with a simple init file (nvim -u ~/path/to/file) with the
" following:
"
" set runtimepath+=~/.config/nvim/plug/unmanaged-plugins/jmin
" colorscheme jmin-white

" TODO: Highlight markdown / HTML quotes, code, and blockquotes same as string
" (when g:jmin_hlstrs is on).

" TODO: Figure out why the vimUserFunc highlight group is behaving oddly. It
" was getting highlighted in grey (nonText colour I think) for a while. Now it
" keeps a white background even with cursorline active. It happens when I have
" cursorline on in two windows showing the same buffer. This does not happen
" in jmin-2color.

" TODO: Figure out why HTML em, strong, i, and b tags become formatted on
" bufEnter (do `:e!`) even when I explicitly set their formatting to NONE
" here. It’s probably something the HTML syntax file does, but I may be able
...
if g:jmin_contrast  == 'high' || g:jmin_contrast == 'max'
    let s:norm_fg       = s:black
endif
if exists("g:jmin_fg")
    let s:norm_fg = g:jmin_fg
endif

let s:norm_bg           = s:white
if exists("g:jmin_bg")
...
" are set to 1.
let s:comment_fg               = s:lighter_grey
...
let s:function                 = s:lighter_orange
...
"let s:directory          = s:deep_sky_blue
let s:directory          = s:norm_fg
let s:symlink            = s:norm_fg
let s:nontext_fg         = s:near_white
let s:nontext_bg         = s:none
let s:signcolumn         = s:grey
"let s:modemsg            = s:dark_sky_blue
...
let s:specialkey         = s:lighter_grey
...
let s:normalfloat_bg                = s:more_extreme_near_white 
let s:floatborder_fg                = s:nearer_white
...
let s:gitgutterchange_fg       = s:light_yellow
let s:gitgutterdelete_fg       = s:bright_red
...
" indent-guides
let s:indent_guides_bg = s:more_extreme_near_white
...
if &t_Co == 8 || g:term_colors == '8' || &t_Co == 16 || g:term_colors == '16' || g:jmin_16 == 1 || s:term == 'linux'
    ...
    " That means you cannot set pure white (color15) as the background colour in
    " the Linux or at least Ubuntu framebuffer terminal. The lightest monotone
    " colour you can set is light grey (color7), so I will explicitly set that for
    " light backgrounds and adjust everything else to work with it. I don’t like
    " this, but the whole point of these 16-colour settings is to make the
    ...
    let s:gitgutterchange_fg   = s:color11
