...
"let s:norm_bg           = s:extreme_near_white
let s:norm_bg           = s:nearest_white
if exists("g:jmin_bg")
...
let s:statusline_nc_bg   = s:nearer_white2
let s:statusline_nc_fg   = s:dim_grey
let s:statusline_nc_attr = s:none
let s:vertsplit          = s:nearer_white

" Search

let s:search_fg      = s:blue
let s:search_bg      = s:palish_yellow
...
let s:cursorline_bg    = s:nearer_white
...
let s:colorcolumn_bg   = s:nearest_white
...
let s:nontext_fg       = s:lighter_grey0
...
let s:warningmsg_fg    = s:lighter_orange
...
let s:diffadd_bg           = s:paler_green
...
let s:diffadded_fg       = s:light_green
...
" Git Gutter
let s:gitgutteradd_fg      = s:light_green
...
" indent-guides
let s:indent_guides_bg = s:dim_white
...
if g:jmin_2color
    ...
    let s:diffadded_bg       = s:none
    let s:diffadded_attr     = s:italic
    let s:diffremoved_fg     = s:none
    let s:diffremoved_bg     = s:none
    let s:diffremoved_attr   = s:italic
...
if &t_Co == 8 || g:term_colors == '8' || &t_Co == 16 || g:term_colors == '16' || g:jmin_16 == 1 || s:term == 'linux'
    ...
    if g:jmin_2color
        ...
        let s:pmenusel_fg              = s:norm_fg
        let s:pmenusel_bg              = s:norm_bg
        " This attribute list gets added to the list in s:pmenu_attr unless
        " you add nocombine.
        let s:pmenusel_attr            = { "cterm": "nocombine,NONE", "gui": "nocombine,NONE" }
        "let s:pmenusel_attr            = s:none
        let s:pmenusbar_fg             = s:norm_fg
