...
" are set to 1.
let s:comment_fg               = s:darker_grey
...
let s:linenr                   = s:darkest_grey
"let s:string                   = s:grey
let s:string                   = s:paler_sky_blue0
if exists("g:jmin_string")
    let s:string               = g:jmin_string
endif
let s:constant                 = s:string
"let s:statement_fg             = s:white
...
let s:statusline_nc_bg   = s:near_black
let s:statusline_nc_fg   = s:darker_grey
let s:statusline_nc_attr = s:none
let s:vertsplit          = s:statusline_nc_bg
...
"let s:cursorline_bg    = s:extreme_near_black
...
let s:colorcolumn_bg   = s:extreme_near_black
...
let s:nontext_fg       = s:very_dark_grey
...
let s:tablinefill      = s:darkest_grey
...
let s:normalfloat_bg                = s:extreme_near_black
...
let s:diffchange_bg            = { "gui": "#001b1b", "cterm": "NONE" }
let s:diffdelete_fg            = s:darker_grey
let s:diffdelete_bg            = s:near_black
...
" indent-guides
let s:indent_guides_bg = s:nearest_black
...
if g:jmin_2color
    ...
    " This attribute list gets added to the list in s:pmenu_attr unless you
    " add nocombine.
    "let s:pmenusel_attr            = s:none
    let s:pmenusel_attr            = { "cterm": "nocombine,NONE", "gui": "nocombine,NONE" }
